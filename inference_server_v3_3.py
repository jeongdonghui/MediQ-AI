# -*- coding: utf-8 -*-
"""
MediQ AI Inference Server v3.3 (CSV-First Tiered Selection Edition)
===================================================================
...
실행 예시:
uvicorn inference_server_v3_3:app --host 0.0.0.0 --port 8000
"""
...
print(f"[MediQ v3.3] 허용 질환 수: {len(ALLOWED_LABELS)}")
print(f"[MediQ v3.3] fallback 후보 수: {len(FALLBACK_DF)}")
...
def fallback_by_csv(
    ...
    print(f"[MediQ v3.3] CSV fallback diseaseName: {disease_name}")
...
def select_best_candidate(
    ...
    for index, candidate in enumerate(candidates):
        final_score = (
            ALPHA_CSV_PRIOR * p_csv[index]
            + (1.0 - ALPHA_CSV_PRIOR) * p_model[index]
        )
    ...
    if (
        ...
        and (model_best_final - csv_top_final) < MODEL_OVERRIDE_MARGIN
    ):
        return csv_top_label, ranked

    return model_best_label, ranked
...
def analyze_constrained(input_json: Dict[str, Any]) -> Dict[str, str]:
    ...
    if detect_red_flag(input_json):
        print(f"[MediQ v3.3] Red-flag 감지 → 응급 우회 (증상: {symptom_log})")
    ...
    print(
        f"[MediQ v3.3] tier={tier} 후보({len(candidates)})={candidates} / "
        f"증상={symptom_log}"
    )
    ...
    if tier == TIER_EXACT:
        chosen = candidates[0]
        print(f"[MediQ v3.3] exact match → CSV 1순위 즉시 선택: {chosen}")
        return make_result_from_disease_name(chosen)

    # 후보가 1개면 모델 호출 없이 선택
    if len(candidates) == 1:
        chosen = candidates[0]
        print(f"[MediQ v3.3] 단일 후보 선택: {chosen}")
        return make_result_from_disease_name(chosen)

    try:
        best, ranked = select_best_candidate(input_json, pairs)
        ...
        print(f"[MediQ v3.3] 하이브리드[{tier}] {detail} -> 선택: {best}")
        ...
        print(
            f"[MediQ v3.3] 스코어링 실패({type(error).__name__}: {error}) "
            "-> CSV fallback"
        )
...
def health_check() -> Dict[str, str]:
    return {
        ...
        "inferenceMode": "csv_first_tiered_then_hybrid",
